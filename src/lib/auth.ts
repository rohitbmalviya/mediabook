'use server'
// ============================================================
// MediBook — Auth server actions
//
// registerAction  → create account (+ DoctorProfile if DOCTOR)
 ...
 * If role=DOCTOR, also creates a starter DoctorProfile.
 * On success, sets the session cookie and redirects to the appropriate dashboard.
 */
  ...
  try {
    const user = await db.user.create({
      data: {
        ...
        name,
        role: role as Role,
        // Create a starter DoctorProfile when registering as a doctor
        ...(role === 'DOCTOR' && {
          doctorProfile: {
            create: {
              specialty: 'General Practice',
              bio: '',
              consultationFee: 50000, // Rs 500 in paise — doctor updates later
              city: '',
